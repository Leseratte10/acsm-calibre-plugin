...
def debug_log(message):
    with open("/tmp/calibre_plugin_debug.log", "a") as f:
        import datetime
...
class ACSMInput(FileTypePlugin):
    ...
    def initialize(self):
        ...
        try:
            ...
            try:
                ...
                print(
                    "{0} v{1}: Couldn't add ACSM to book extension list:".format(
            ...
            if singleinstance("__acsm_rename_old_plugin"):
                ...
                if os.path.exists(os.path.join(self.pluginsdir, "DeACSM.zip")):
                    os.rename(
                        os.path.join(self.pluginsdir, "DeACSM.zip"),
                        os.path.join(self.pluginsdir, "DeACSM.BAK"),
                    )
            ...
            try:
                ...
                print(
                    "{0} v{1}: Couldn't initialize GUI plugin:".format(
                        PLUGIN_NAME, PLUGIN_VERSION
                    )
                )
                traceback.print_exc()
            ...
            try:
                ...
                id_plugin = (
                    ts_dict["module_id.txt"].decode("latin-1").split("\n")[0].strip()
                )
            except:
            ...
            if id is None or id_plugin is None or id != id_plugin:
                print(
                    'Module update from "{0}" to "{1}", extracting ...'.format(
                        id, id_plugin
                    )
                )
                ...
                if not singleinstance("__acsm_extracting modules"):
                    ...
                    for entry, data in lib_dict.items():
                        try:
                            ...
                            print(
                                "{0} v{1}: Exception when copying needed library files".format(
                                    PLUGIN_NAME, PLUGIN_VERSION
                                )
                            )
                            traceback.print_exc()
            ...
            if platform.system() == "Darwin":
                ...
                if libcrypto_path is None:
                    libcrypto_path = get_launchctl_env("ACSM_LIBCRYPTO")
                if libssl_path is None:
                    libssl_path = get_launchctl_env("ACSM_LIBSSL")
            ...
            elif platform.system() == "Darwin" and check_homebrew_openssl():
                # MacOS with Homebrew muust specify Apple signed libraries
                libcrypto_path, libssl_path = get_mac_crypto_libs()
    ...
    def download(self, replyData):
        ...
        try:
            download_url = adobe_fulfill_response.find(
                "./%s/%s/%s"
                % (adNS("fulfillmentResult"), adNS("resourceItemInfo"), adNS("src"))
            ).text
        except:
            print(
                "{0} v{1}: FulfillmentResult does not contain the <src> tag. This may be an ACSM with download type 'auth'?".format(
                    PLUGIN_NAME, PLUGIN_VERSION
                )
            )
            print(
                "{0} v{1}: Please download the book through ADE (so the ACSM file is 'used').".format(
                    PLUGIN_NAME, PLUGIN_VERSION
                )
            )
            print(
                "{0} v{1}: After that, please open a bug report and attach the ACSM file.".format(
                    PLUGIN_NAME, PLUGIN_VERSION
                )
            )
            return None
        ...
        license_token_node = adobe_fulfill_response.find(
            "./%s/%s/%s"
            % (
                adNS("fulfillmentResult"),
                adNS("resourceItemInfo"),
                adNS("licenseToken"),
            )
        )
        ...
        if rights_xml_str is None:
            print(
                "{0} v{1}: Building rights.xml failed!".format(
                    PLUGIN_NAME, PLUGIN_VERSION
                )
            )
            return None
        ...
        # Download eBook:
        print(
            "{0} v{1}: Loading book from {2}".format(
                PLUGIN_NAME, PLUGIN_VERSION, download_url
            )
        )
        ...
        if ret != 200:
            print(
                "{0} v{1}: Download failed with error {2}".format(
                    PLUGIN_NAME, PLUGIN_VERSION, ret
                )
            )
            return None
        ...
        if filetype == ".epub":
            ...
            zf.close()
            print(
                "{0} v{1}: File successfully fulfilled ...".format(
                    PLUGIN_NAME, PLUGIN_VERSION
                )
            )
            return filename
            ...
            adNS = lambda tag: "{%s}%s" % ("http://ns.adobe.com/adept", tag)
            resource = adobe_fulfill_response.find(
                "./%s/%s" % (adNS("licenseToken"), adNS("resource"))
            ).text

            print(
                "{0} v{1}: Downloaded PDF, adding encryption config ...".format(
                    PLUGIN_NAME, PLUGIN_VERSION
                )
            )
            ...
            if ret:
                ...
                print(
                    "{0} v{1}: There was an error patching the PDF file.".format(
                        PLUGIN_NAME, PLUGIN_VERSION
                    )
                )
        ...
        while True:
            with SingleInstance("__acsm_plugin_execute_run_acsm_file") as si:
                if si:
                    ...
                    if counter % 100 == 0:
                        print(
                            "Thread {0} still waiting for lock, attempt {1}".format(
                                thread_id, counter
                            )
                        )
                    time.sleep(0.1)
    ...
    def run_single(self, path_to_ebook):
        ...
        try:
            ...
            print(
                "{0} v{1}: Trying to parse file {2}".format(
            ...
            if ext != ".acsm":
                print(
                    "{0} v{1}: That's not an ACSM, returning (is {2} instead)... ".format(
                        PLUGIN_NAME, PLUGIN_VERSION, ext
                    )
                )
            ...
            if not self.ADE_sanity_check():
                print(
                    "{0} v{1}: ADE auth is missing or broken ".format(
                        PLUGIN_NAME, PLUGIN_VERSION
                    )
                )
                return path_to_ebook
            ...
            if not are_ade_version_lists_valid():
                print(
                    "{0} v{1}: ADE version list mismatch, please open a bug report.".format(
                        PLUGIN_NAME, PLUGIN_VERSION
                    )
                )
                return path_to_ebook
            ...
            success, replyData = fulfill(
                path_to_ebook, deacsmprefs["notify_fulfillment"]
            )

            if success is False:
                print(
                    "{0} v{1}: Hey, that didn't work: \n".format(
                        PLUGIN_NAME, PLUGIN_VERSION
                    )
                    + replyData
                )
                ...
                print(
                    "{0} v{1}: Downloading book ...".format(PLUGIN_NAME, PLUGIN_VERSION)
                )
                ...
                if rpl is not None:
                    ...
                    try:
                        ...
                        for plugin in _initialized_plugins:
                            ...
                            # don't execute it.
                            my_file_type = (
                                os.path.splitext(rpl)[-1].lower().replace(".", "")
                            )
                            ...
                            print(
                                "{0} v{1}: Executing plugin {2} ...".format(
                                    PLUGIN_NAME, PLUGIN_VERSION, plugin.name
                                )
                            )
                            ...
                            try:
                                ...
                                print(
                                    "{0} v{1}: Running file type plugin failed with traceback:".format(
                                        PLUGIN_NAME, PLUGIN_VERSION
                                    )
                                )
                            ...
                            if plugin_ret is not None:
                                ...
                                print(
                                    "{0} v{1}: Plugin returned path '{2}', updating.".format(
                                        PLUGIN_NAME, PLUGIN_VERSION, plugin_ret
                                    )
                                )
                                rpl = plugin_ret
                            else:
                                print(
                                    "{0} v{1}: Plugin returned nothing - skipping".format(
                                        PLUGIN_NAME, PLUGIN_VERSION
                                    )
                                )
                        ...
                        delete_src_file = False
                        print(
                            "{0} v{1}: Error while executing other plugins".format(
                                PLUGIN_NAME, PLUGIN_VERSION
                            )
                        )
                        traceback.print_exc()
                    ...
                    if delete_src_file:
                        try:
                            if os.path.exists(path_to_ebook):
                                print(
                                    "{0} v{1}: Deleting existing ACSM file {2} ...".format(
                                        PLUGIN_NAME, PLUGIN_VERSION, path_to_ebook
                                    )
                                )
                                os.remove(path_to_ebook)
                        except:
                            print(
                                "{0} v{1}: Failed to delete source ACSM after fulfillment.".format(
                                    PLUGIN_NAME, PLUGIN_VERSION
                                )
                            )
...
def test_libs(l_crypto, l_ssl):
    wipe_crypto()  # because might not have restarted calibre
    try:
...
def get_launchctl_env(var_name):
    import subprocess
